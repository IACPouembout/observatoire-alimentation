...
header_img <- div(
  ...
  div(
    class = "my-title",
    h4('Observatoire des systèmes alimentaires en Nouvelle-Calédonie'),
...
ui<- dashboardPage( skin = "black",
                    ...
                    dashboardSidebar(width = 350,sidebarMenu(
                      ...
                      menuItem("Production et prix des fruits et légumes",tabName = "prix_prod",icon = icon("chart-line")),
                      menuItem("Géographie des prix des fruits et légumes",tabName = "map_prix",icon = icon("map")),
               ...
               tabItems(
                 tabItem(tabName = "presentation",mainPanel(
                   ...
                   fluidPage(
                     ...
                     p("Bienvenue sur l'observatoire des sytèmes alimentaires",style="font-size:20px;"),
                     p("Ce tableau de bord a été crée dans le cadre du projet TRIAD pour permettre un suivi temporel et spatial des prix des produits alimentaires, en faisant le lien avec les données de ventes et d'importations."),
                     p("Ce dernier utilise les données de prix.nc de suivi des prix, les données de la DAVAR de suivi des productions et ventes, les données des enquêtes IAC, ainsi que la Pacific Food Trade Database de la Communauté du Pacifique Sud (CPS)."),
                     tags$div(tags$ul(
                       tags$li(tags$span(icon("chart-line"),"L'onglet 'Production et prix des fruits et légumes' vous permet de suivre la production, les importations (uniquement à partir de 2021) et l'évolution des prix des fruits et légumes, sur la période 2011-2023." ,br(),"L'utilisateur peut également afficher les cyclones ayant eu lieu sur la période, la liste de ces derniers est issue de la base cyclonique de Météo France.",br(), "L'effet de l'évolution de la production sur les prix est représenté graphiquement et est calculé par régression linéaire.")),
                 ...
                 tabItem(
                   tabName = "prix_prod",  
                   fluidPage(h1("Production et prix des fruits et légumes"),   
  ...
  output$map_agri_destinations <- renderLeaflet({
    leaflet(data = map_agri_tribu_region_total)%>%
      ...
      addMinicharts(map_agri_tribu_region_total$Long,map_agri_tribu_region_total$Lat,
                    ...
                    chartdata = as_tibble(map_agri_tribu_region_total)[5:10],
      )})
  ...
  observe({
    ...
    leafletProxy("map_agri_destinations")%>%
      updateMinicharts(layerId = map_agri_tribu_region$N_region,type = "pie",colorPalette = colors,
                       chartdata = as_tibble(map_agri_tribu_region)[5:10])
